...
3. Use `/react-doctor` from `.codex/prompts/react-doctor.md` when a task asks for React Doctor triage or broad React cleanup.
4. Validate that first slice immediately and stop for feedback before expanding to the next slice.
5. Keep edits scoped and explicit.
6. Prefer `npm run commit:with-progress -- "<message>" --learning "<learning>" -- "<path>" ...` for agent-requested commits so `progress.md` is appended in the same commit.
7. Use `npm run commit:selective -- "<message>" "<path>" ...` when a commit is intentionally not part of the learning-log flow.
8. Keep public package APIs re-exported through `src/index.ts`.

## Validate

1. Run `npm run check` for code quality.
2. Run `npm run react:doctor:changed` after React or Next.js changes in `apps/web`.
3. Run `npm test` for test coverage.
4. Run `npm run agent:check` before ship or handoff to validate docs, AGENTS structure, skill front matter, and vendored sync across all skill packs.
